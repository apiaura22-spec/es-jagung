...
<div class="d-flex">
    <div class="sidebar shadow" id="sidebar">
        <div class="sidebar-header">
            ...
            <div class="badge bg-dark-subtle text-muted mt-2 border border-secondary" style="font-size: 0.65rem;">ADMIN PANEL</div>
        </div>
    ...
    <div class="main-wrapper">
        <nav class="top-navbar d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center">
                ...
                <h5 class="fw-bold text-dark mb-0 d-none d-sm-block">@yield('title')</h5>
            </div>
            ...
            <div class="dropdown">
                <div class="d-flex align-items-center gap-3 cursor-pointer" data-bs-toggle="dropdown">
                    <div class="text-end d-none d-md-block">
                        ...
                        <p class="mb-0 text-success fw-medium" style="font-size: 0.65rem;"><i class="fa-solid fa-circle me-1" style="font-size: 0.4rem;"></i> Online</p>
                    </div>
                    <div class="admin-avatar">{{ strtoupper(substr(Auth::user()->name, 0, 1)) }}</div>
                </div>
                <ul class="dropdown-menu dropdown-menu-end shadow-lg border-0 rounded-4 p-2 animate__animated animate__fadeIn">
                    <li><a class="dropdown-item rounded-3" href="#"><i class="fa-solid fa-user-gear me-2"></i> Profil</a></li>
                    <li><hr class="dropdown-divider"></li>
                    <li>
                        <form action="{{ route('logout') }}" method="POST">
                            @csrf
                            <button type="submit" class="dropdown-item text-danger rounded-3"><i class="fa-solid fa-right-from-bracket me-2"></i> Keluar</button>
                        </form>
                    </li>
                </ul>
            </div>
        ...
        <footer class="px-5 py-4 text-center text-muted small border-top bg-white mt-auto">
            <span>&copy; {{ date('Y') }} Es Jagung Uni Icis. Crafted with <i class="fa-solid fa-heart text-danger"></i></span>
        </footer>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        // Realtime System
        let lastContent = $('#realtime-container').html().trim();
        const notificationSound = new Audio('https://assets.mixkit.co/active_storage/sfx/2358/2358-preview.mp3');

        function checkUpdates() {
            $.get(window.location.href, function(data) {
                const newHtml = $(data).find('#realtime-container').html();
                
                if (newHtml.trim() !== lastContent) {
                    $('#realtime-container').html(newHtml);
                    lastContent = newHtml.trim();
                    
                    // Notifikasi Modern saat ada data baru
                    Toast.fire({
                        icon: 'info',
                        title: 'Data diperbarui secara otomatis'
                    });
